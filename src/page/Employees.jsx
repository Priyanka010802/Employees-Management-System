...
const ManageEmployees = () => {
  ...
  const [selectedEmployeeForPrint, setSelectedEmployeeForPrint] =
    useState(null);
          ...
          <div class="content">
            ...
            <div class="details-grid">
              <div class="detail-card">
                ...
                <div class="detail-value salary-value">
                  ₹${
                    employee?.salary
  ...
  return (
    <div className="flex-1 min-h-screen bg-slate-50 relative">
      ...
      <header className="relative z-10 h-16 border-b border-slate-200/70 flex items-center px-4 md:px-8 bg-white/80 backdrop-blur-2xl">
        <div className="flex flex-col">
          ...
          <p className="text-xs md:text-sm text-slate-500">
            Employee Management System • Employee Directory
          </p>
        </div>
      </header>
      ...
      <main className="relative z-10 flex-1 px-4 md:px-8 py-6 md:py-8 max-w-6xl mx-auto space-y-6">
        {/* page header row */}
        <section
          className="rounded-3xl bg-white/90 backdrop-blur-2xl border border-slate-200 shadow-sm px-5 md:px-8 py-5 flex flex-col gap-4"
          data-aos="fade-down"
        >
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            ...
            <div className="flex items-center gap-3">
              ...
              <button
                ...
                className="inline-flex items-center gap-1.5 rounded-2xl bg-sky-500 text-white text-xs md:text-sm font-semibold px-4 py-2 shadow-sm hover:bg-sky-600 transition-all"
              >
        ...
        {/* add / edit form (drawer-style card above grid) */}
        {isFormOpen && (
          <section
            className="relative rounded-3xl border border-slate-200 bg-white/95 backdrop-blur-2xl shadow-md px-5 md:px-6 py-5"
            data-aos="fade-up"
          >
            <div className="flex items-start justify-between gap-4 mb-4">
              <div>
                ...
                <p className="text-xs md:text-sm text-slate-500 mt-1">
                  Fill employee details. You can update them later.
                </p>
              </div>
              <button
                onClick={() => {
        ...
        {/* directory grid like screenshot */}
        <section
          className="rounded-3xl bg-white/90 backdrop-blur-2xl border border-slate-200 shadow-sm px-4 md:px-6 py-5"
          data-aos="fade-up"
        >
          {filtered.length === 0 ? (
            <div className="py-12 text-center text-sm text-slate-500">
              No employees found. Try a different search or add a new
              employee.
            </div>
          ) : (
              ...
              {filtered.map((emp) => {
                const imgSrc =
                  emp.image && emp.image.trim() !== ""
                    ? emp.image
                    : fallbackAvatar(emp.name);

                return (
                  <div
                    key={emp.id}
                    className="rounded-2xl border border-slate-200 bg-slate-50/60 hover:bg-white shadow-sm hover:shadow-md transition-all duration-200 flex flex-col"
                  >
                    <div className="p-4 border-b border-slate-100 flex items-start gap-3">
                      <div className="relative">
                        <img
                          ...
                          onError={(e) => {
                            e.currentTarget.src = fallbackAvatar(emp.name);
                          }}
                        />
                    ...
                    <div className="px-4 py-3 text-[11px] text-slate-500 space-y-2 flex-1">
                      <div className="flex justify-between gap-4">
                        <div>
                          ...
                          <p className="text-slate-700 truncate max-w-[150px]">
                            {emp.email}
                          </p>
                        </div>
                    ...
                    <div className="px-4 pb-3 pt-2 border-t border-slate-100 flex items-center justify-between">
                      <div className="flex gap-1.5 text-[11px]">
                        ...
                        <button
                          ...
                          className="px-2 py-1 rounded-full border border-sky-200 text-sky-600 hover:bg-sky-50"
                        >
                          Edit
                        </button>
                        <button
                          onClick={() => handleDelete(emp.id)}
                          className="px-2 py-1 rounded-full border border-rose-200 text-rose-600 hover:bg-rose-50"
                        >
                          Delete
                        </button>
                      </div>
            ...
            <div className="p-5 md:p-6 max-w-3xl mx-auto overflow-y-auto">
              ...
              <div className="flex flex-col lg:flex-row gap-6 mb-8">
                <div className="flex justify-center lg:justify-start flex-shrink-0">
                  <img
                    src={
                      selectedEmployeeForPrint.image ||
                      fallbackAvatar(selectedEmployeeForPrint.name)
                    }
                ...
                <div className="flex-1 space-y-3">
                  <div>
                    ...
                    <p className="text-sm md:text-base text-slate-600 font-medium">
                      {selectedEmployeeForPrint.role} •{" "}
                      {selectedEmployeeForPrint.department}
                    </p>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    <div className="p-4 rounded-2xl bg-indigo-50 border border-indigo-100">
                      <span className="block text-[11px] font-semibold text-indigo-700 uppercase tracking-wide mb-1">
                        Employee ID
                      </span>
                      <span className="text-lg font-semibold text-slate-900">
                        {selectedEmployeeForPrint.empCode ||
                          (selectedEmployeeForPrint.id
                            ? `EMP-${String(
                                selectedEmployeeForPrint.id
                              ).padStart(3, "0")}`
                            : "N/A")}
                    ...
                    <div className="p-4 rounded-2xl bg-emerald-50 border border-emerald-100">
                      <span className="block text-[11px] font-semibold text-emerald-700 uppercase tracking-wide mb-1">
                        Hiring date
                      </span>
                      <span className="text-lg font-semibold text-slate-900">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-5 mb-8">
                <div className="p-5 rounded-2xl bg-emerald-50 border border-emerald-100">
                  <span className="block text-[11px] font-semibold text-emerald-700 uppercase tracking-wide mb-2">
                    Salary (per month)
                  </span>
                  <span className="text-2xl md:text-3xl font-semibold text-emerald-700">
                    ₹
                    {selectedEmployeeForPrint.salary
                      ? Number(
                          selectedEmployeeForPrint.salary
                        ).toLocaleString()
                      : "0"}
                ...
                <div className="p-5 rounded-2xl bg-sky-50 border border-sky-100">
                  <span className="block text-[11px] font-semibold text-sky-700 uppercase tracking-wide mb-2">
                    Department
                  </span>
                ...
                <div className="lg:col-span-2 p-5 rounded-2xl bg-slate-50 border border-slate-200">
                  <span className="block text-[11px] font-semibold text-slate-700 uppercase tracking-wide mb-2">
                    Office address
                  </span>
