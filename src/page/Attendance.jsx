...
// ✅ FIXED: Define sampleEmployees locally (no import needed)
...
const Attendance = () => {
  ...
  const [stats, setStats] = useState({
    total: 0, present: 0, absent: 0, late: 0, avgWeekly: 0, avgMonthly: 0
  });
  ...
  const calculateStats = (records) => {
    ...
    const avgWeekly = weeklyRecords.length > 0 ? 
      (weeklyRecords.filter(r => r.status === 'Present').length / weeklyRecords.length * 100).toFixed(1) : 0;
    const avgMonthly = monthlyRecords.length > 0 ? 
      (monthlyRecords.filter(r => r.status === 'Present').length / monthlyRecords.length * 100).toFixed(1) : 0;
  ...
  return (
    <div className="flex-1 min-h-screen bg-gradient-to-br from-indigo-900 via-purple-900 to-pink-900 overflow-hidden p-6 md:p-10 relative">
      ...
      <div className="max-w-7xl mx-auto space-y-8 relative z-10">
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
          <div>
            ...
            <div className={`inline-flex items-center gap-2 mt-2 px-4 py-2 rounded-2xl text-sm font-bold border ${
              serverStatus === 'connected' ? 'bg-emerald-500/20 text-emerald-300 border-emerald-400/50' :
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          <div className="group relative bg-white/10 backdrop-blur-3xl border border-white/20 rounded-3xl p-8 hover:bg-white/20 transition-all duration-500 hover:scale-[1.02] shadow-2xl hover:shadow-3xl cursor-pointer">
            ...
            <div className="relative z-10">
              ...
              <div className="text-slate-300 text-sm font-semibold mt-1">Total Records</div>
            </div>
          </div>
          ...
          <div className="group relative bg-white/10 backdrop-blur-3xl border border-white/20 rounded-3xl p-8 hover:bg-emerald-500/20 transition-all duration-500 hover:scale-[1.02] shadow-2xl hover:shadow-3xl">
            ...
            <div className="relative z-10">
              <div className="text-3xl">✅</div>
              <div className="text-2xl font-black text-emerald-300 mt-2">{stats.present}</div>
              <div className="text-emerald-200 text-sm font-semibold mt-1">Present</div>
            </div>
          </div>
          ...
          <div className="group relative bg-white/10 backdrop-blur-3xl border border-white/20 rounded-3xl p-8 hover:bg-rose-500/20 transition-all duration-500 hover:scale-[1.02] shadow-2xl hover:shadow-3xl">
            ...
            <div className="relative z-10">
              <div className="text-3xl">❌</div>
              <div className="text-2xl font-black text-rose-300 mt-2">{stats.absent}</div>
              <div className="text-rose-200 text-sm font-semibold mt-1">Absent</div>
            </div>
          </div>
          ...
          <div className="group relative bg-white/10 backdrop-blur-3xl border border-white/20 rounded-3xl p-8 hover:bg-amber-500/20 transition-all duration-500 hover:scale-[1.02] shadow-2xl hover:shadow-3xl">
            ...
            <div className="relative z-10">
              <div className="text-3xl">⏰</div>
              <div className="text-2xl font-black text-amber-300 mt-2">{stats.late}</div>
              <div className="text-amber-200 text-sm font-semibold mt-1">Late Comers</div>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="bg-white/10 backdrop-blur-3xl border border-white/20 rounded-3xl p-8">
            <div className="flex items-center gap-4 mb-6">
              <div className="w-16 h-16 bg-gradient-to-br from-blue-500 to-purple-500 rounded-2xl flex items-center justify-center text-2xl font-bold text-white shadow-2xl">📈</div>
              <h3 className="text-2xl font-black text-white">Weekly Average</h3>
            </div>
            <div className="text-4xl font-black text-blue-300">{stats.avgWeekly}%</div>
            <div className="text-slate-300 mt-2 text-lg">Present this week</div>
          ...
          <div className="bg-white/10 backdrop-blur-3xl border border-white/20 rounded-3xl p-8">
            <div className="flex items-center gap-4 mb-6">
              <div className="w-16 h-16 bg-gradient-to-br from-emerald-500 to-teal-500 rounded-2xl flex items-center justify-center text-2xl font-bold text-white shadow-2xl">📅</div>
              <h3 className="text-2xl font-black text-white">Monthly Average</h3>
            </div>
            <div className="text-4xl font-black text-emerald-300">{stats.avgMonthly}%</div>
            <div className="text-slate-300 mt-2 text-lg">Present this month</div>
            <div className="w-full bg-slate-800 rounded-full h-3 mt-4 overflow-hidden">
              <div 
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead className="bg-white/5 backdrop-blur-xl">
                <tr>
                  <th className="px-6 py-4 text-left font-bold text-white/90 text-sm uppercase tracking-wider">Date</th>
                  <th className="px-6 py-4 text-left font-bold text-white/90 text-sm uppercase tracking-wider">Employee</th>
                  <th className="px-6 py-4 text-left font-bold text-white/90 text-sm uppercase tracking-wider">Status</th>
                  <th className="px-6 py-4 text-left font-bold text-white/90 text-sm uppercase tracking-wider">Check-in</th>
                  <th className="px-6 py-4 text-left font-bold text-white/90 text-sm uppercase tracking-wider">Check-out</th>
                  <th className="px-6 py-4 text-left font-bold text-white/90 text-sm uppercase tracking-wider">Actions</th>
                </tr>
              </thead>
              <tbody>
                  ...
                  return (
                    <tr key={record.id} className="border-b border-white/10 hover:bg-white/10 transition-all">
                      <td className="px-6 py-4">
                        <div className="font-semibold text-white/90">{record.date}</div>
                      </td>
                      <td className="px-6 py-4">
                        <div className="font-semibold text-white/90 max-w-xs truncate">
                          {record.employeeName}
                        </div>
                        <div className="text-xs text-slate-400">{record.employeeId}</div>
                        <div className="text-xs bg-gradient-to-r from-emerald-500/30 to-blue-500/30 text-emerald-200 px-2 py-0.5 rounded-full mt-1">
                          {empStats.presentRate}% Present Rate
                        </div>
                      </td>
                      <td className="px-6 py-4">
                        <span className={`px-4 py-2 rounded-full text-xs font-bold text-white ${
                          record.status === 'Present' ? 'bg-emerald-500/90 shadow-emerald-500/50' :
                          record.status === 'Absent' ? 'bg-rose-500/90 shadow-rose-500/50' :
                          'bg-amber-500/90 shadow-amber-500/50'
                        }`}>
                          {record.status}
                        </span>
                      </td>
                      <td className="px-6 py-4 text-white/80 font-mono text-sm">{record.checkIn || '—'}</td>
                      <td className="px-6 py-4 text-white/80 font-mono text-sm">{record.checkOut || '—'}</td>
                      <td className="px-6 py-4">
                  ...
                  <tr>
                    <td colSpan={6} className="p-20 text-center">
                      <div className="flex flex-col items-center gap-6 text-white/60">
                        ...
                        <div>
                          <h3 className="text-2xl font-bold mb-3">No attendance records yet</h3>
                          <p className="text-lg">Click "Mark Attendance" to get started</p>
                        </div>
                      </div>
                    </td>
                  </tr>
      ...
      {/* Attendance Modal */}
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-6 bg-black/70 backdrop-blur-md" onClick={closeModal}>
          <div 
          ...
          >
            ...
            <form onSubmit={editingRecord ? updateAttendance : addAttendance} className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  <label className="block text-sm font-bold text-slate-300 mb-2">Employee</label>
                  <select 
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div>
                  <label className="block text-sm font-bold text-slate-300 mb-2">Check-in</label>
                  <input
                    type="time"
                    value={checkIn}
                ...
                <div>
                  <label className="block text-sm font-bold text-slate-300 mb-2">Check-out</label>
                  <input
                    type="time"
                    value={checkOut}
